...
// Sort objects by Kind then Name
func sortByKind(src []runtime.Object) []runtime.Object {
	var tgt []runtime.Object
	copy(tgt, src)

	fmt.Printf("%+v\n", src)
	fmt.Printf("%+v\n", tgt)
	sort.Slice(tgt, func(i, j int) bool {
		iMeta := tgt[i].(metav1.Object)
		jMeta := tgt[j].(metav1.Object)
		iType := tgt[i].GetObjectKind().GroupVersionKind().Kind
		jType := tgt[j].GetObjectKind().GroupVersionKind().Kind
